# This file is adapted from HOLD. Original code: https://github.com/zc-alexfan/hold
import numpy as np
import torch
import os
import utils.data_util as thing
...
class xdict(dict):
    ...
    def overwrite(self, k, v):
        ...
        """
        assert k in self.keys(), f"Key does not exist {k}"
        super().__setitem__(k, v)
    ...
    def to_16_bits(self):
        ...
        for k, v in self.items():
            if isinstance(v, dict):
                self.overwrite(k, xdict(v).to_16_bits())
            if not isinstance(v, torch.Tensor):
                continue
            dtype = v.dtype
            if dtype in [torch.float32, torch.float64]:
                self.overwrite(k, v.to(torch.float16))
            elif dtype == torch.int64:
                self.overwrite(k, v.to(torch.int16))
        return self

    def to_std_precision(self):
        """
        ...
        for k, v in self.items():
            ...
            if dtype in [torch.float16, torch.float64]:
                self.overwrite(k, v.to(torch.float32))
            elif dtype in [torch.int16, torch.int32]:
                self.overwrite(k, v.to(torch.int64))
        return self