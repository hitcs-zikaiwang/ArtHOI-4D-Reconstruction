...
def pack_dataset(seq_path, part_cnt, use_vi_camera=True):
    ## load Cameras (works for both vi and raw)
    if use_vi_camera:
        camera_path = os.path.join(seq_path, 'inpainting', 'camera_param.npy')
    else:
        camera_path = os.path.join(seq_path, 'camera_param.npy')
    ...
    masks = {}
    for k in mask_keys:
        masks[k], _ = load_masks_from_path(os.path.join(seq_path, 'mask', k), img_size)
    ## load rgbs 
    rgb_path = f'{seq_path}/image/*.png'
    rgbs, frame_ids = load_rgbs_from_path(rgb_path, img_size)
    # load depth
    depth_path = os.path.join(seq_path, 'metric_depth.pkl')
    ...
    img_mask_cams_dep_frameID(
        ...
        depths,
        frame_ids,
        f'{seq_path}/../packed/visuals.npz'
    )
    
    # vi RGB & masks
    vi_rgb_path = f'{seq_path}/inpainting/image/*.png'
    vi_rgbs, vframe_ids = load_rgbs_from_path(vi_rgb_path, img_size)
    vi_masks = {}
    for k in mask_keys:
        if k != 'human' and k != 'composite' and k != 'hand':
            vi_masks[k], _ = load_masks_from_path(os.path.join(seq_path, 'inpainting', 'mask', k), img_size)
    # vi depth
    vi_depth_path = os.path.join(seq_path, 'inpainting', 'metric_depth.pkl')
    ...
    img_mask_cams_dep_frameID(
        ...
        vi_depths,
        vframe_ids,
        f'{seq_path}/../packed/visuals_inpainting.npz'
    )