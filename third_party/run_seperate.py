...
#### Export to file #####
def reorder_VF(V, F, colors=None):
    """
    Reorders vertices and faces to remove unused vertices and re-index.
    It is assumed that F contains vertex indices that are valid for indexing into V.

    Parameters:
    - V (np.ndarray): Original vertices array of shape (N, 3).
    - F (np.ndarray): Original faces array of shape (M, 3), containing indices into V.
    - colors (np.ndarray, optional): Original FACE colors array of shape (M, C)
                                     (e.g., C=3 for RGB, C=4 for RGBA),
                                     corresponding 1-to-1 with faces in F.

    Returns:
    - V_re (np.ndarray): Reordered vertices (subset of V that are used by F). Shape (N_re, 3).
    - F_re (np.ndarray): Reordered faces with vertex indices updated to map to V_re.
                         The order of faces in F_re is the same as in F. Shape (M, 3).
    - colors_re (np.ndarray, optional): Face colors corresponding to F_re.
                                        If input `colors` were provided, this is
                                        identical to `colors`. Shape (M, C).
    """
    if F.size == 0: # No faces, return empty arrays
        if colors is not None:
            # If F is empty, colors (face colors) must also be empty (shape (0,C)).
            return np.empty((0, V.shape[1]), dtype=V.dtype), \
                   np.empty((0, F.shape[1]), dtype=F.dtype), \
                   np.empty((0, colors.shape[1]), dtype=colors.dtype)
        else:
            return np.empty((0, V.shape[1]), dtype=V.dtype), \
                   np.empty((0, F.shape[1]), dtype=F.dtype)

    unique_vert_idx = np.unique(F.flatten())
    V_re = V[unique_vert_idx]

    # Create a mapping from old vertex indices (those in unique_vert_idx) to new (0 to n-1)
    idx_map = {old_idx: new_idx for new_idx, old_idx in enumerate(unique_vert_idx)}

    # Re-index faces
    F_re = np.array([[idx_map[v_idx] for v_idx in face] for face in F], dtype=F.dtype)

    if colors is not None:
        colors_re = colors # Face colors correspond to F, and F_re preserves face order.
        return V_re, F_re, colors_re, unique_vert_idx
    else:
        return V_re, F_re, unique_vert_idx
    
def seperate_mesh_ply(V, F, FL, orig_colors, filename_base):
    assert V.shape[1] == 3
    assert F.shape[1] == 3
    ...
    for label_cnt, cur_label in enumerate(unique_labels):
        hit_faces_idx = np.where(FL == cur_label)[0]
        hit_faces = F[hit_faces_idx]
        hit_colors = orig_colors[hit_faces_idx]
        # Create a new mesh for the current label
        hit_verts_re, hit_faces_re, hit_colors_re, v_map_to_source = reorder_VF(
            V, hit_faces, colors=hit_colors
        )
        
        # Export the mesh to a PLY file
        mesh = trimesh.Trimesh(vertices=hit_verts_re, faces=hit_faces_re)
        mesh.visual.face_colors = hit_colors_re
        filename = f"{filename_base}_part_{label_cnt}.ply"
        mesh.export(filename)
        np.save(f"{filename_base}_part_{label_cnt}_vmap.npy", v_map_to_source)
    print(f"Exported {len(unique_labels)} segmented meshes to {filename_base}_part_*.ply")
...
def solve_clustering(input_fname, uid, view_id, save_dir="test_results1", out_render_fol= "test_render_clustering", use_agglo=False, max_num_clusters=18, is_pc=False, option=1, with_knn=True, export_mesh=True):
    ...
    if not use_agglo:
        ...
        print("Number of clusters found: ", len(hierarchical_labels))
        
        # NOTE here we only consider do once `max_num_clusters` seperation
        n_cluster = 0
        print("Processing cluster: "+str(n_cluster))
        
        FL = np.array(hierarchical_labels[n_cluster])
        unique_labels = np.unique(FL)
        print("Seperate to: ", len(unique_labels), "clusters")
        ...
        V = mesh.vertices
        F = mesh.faces
        orig_color = mesh.visual.face_colors

        filename_base = os.path.join(out_render_fol, uid)
        seperate_mesh_ply(V, F, FL, orig_color, filename_base)
        # FIXME
...
if __name__ == '__main__':
    ...
    uid = model_name.split(".")[-2]
    view_id = 0
    solve_clustering(
        SOURCE_MODEL, uid, view_id, 
        save_dir=root, out_render_fol= OUTPUT_FOL, 
        use_agglo=USE_AGGLO, 
        max_num_clusters=NUM_CLUSTERS, 
        is_pc=IS_PC, option=OPTION, 
        with_knn=WITH_KNN, export_mesh=EXPORT_MESH
    )
